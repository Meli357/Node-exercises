...
async function getResults() {
  try {
    const firstPlayer = await luckyDraw("Tina");
    console.log("Tina:", firstPlayer);

    const secondPlayer = await luckyDraw("Jorge");
    console.log("Jorge:", secondPlayer);

    const thirdPlayer = await luckyDraw("Julien");
    console.log("Julien:", thirdPlayer);
  } catch (error) {
    console.error(error);
  }
}
